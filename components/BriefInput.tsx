...
import { HOME_BRIEFS } from '@/lib/edition/examples';

export function BriefInput({ initial, onSubmit }: { initial: string; onSubmit: (brief: string) => void }) {
  const [value, setValue] = useState(initial);
  // A FIXED lineup, identical on every visit (no daily rotation / live wire) so each example's
  // generated edition stays cached and replays instantly on a later run.
  const examples = HOME_BRIEFS;
